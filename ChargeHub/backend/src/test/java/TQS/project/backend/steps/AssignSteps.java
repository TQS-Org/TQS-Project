...
public class AssignSteps {
    ...
    public void i_click_on_create_operator() {
        WebElement button = driver.findElements(By.className("create-card")).stream()
    ...
    public void iCreateANewStationWithValidDetails() {
        ...
        wait.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector(".modal.success-animation")));
    }
    ...
    public void iAssignTheStationToAnOperator() {
        ...
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".operator-card:not(.create-card)")));

        // Find operator cards (excluding .create-card)
        List<WebElement> operatorCards = driver.findElements(By.cssSelector(".operator-card:not(.create-card)"));
    ...
    public void iShouldSeeASuccessMessage() {
        ...
        assertTrue(successMessage.getText().contains("assigned") ||
