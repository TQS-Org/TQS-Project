...
public class BookingSteps {
  ...
  public void viewListOfChargers() {
    List<WebElement> chargers = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector(".charger-list .charger-card")));
  ...
  public void clickTheFirstCharger() {
    List<WebElement> chargerCards = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.cssSelector(".charger-list .charger-card")));
  ...
  public void viewBookingsForToday(String statusValue) {
      ...
      Assertions.assertEquals(statusValue, statusBadge.getText(), "Status text does not match expected value");
  }
  ...
  public void selectNextDay() {
    ...
    ((JavascriptExecutor) driver).executeScript("arguments[0].value = arguments[1]; arguments[0].dispatchEvent(new Event('input', { bubbles: true }));", dateInput, nextDayStr);
  }
  ...
  public void fillBookingForm(io.cucumber.datatable.DataTable dataTable) {
    ...
    for (List<String> row : data) {
      ...
      if (field.contains("starttime")) {
        WebElement select = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("form#booking-form select")));
        select.click();
        WebElement option = select.findElement(By.xpath(".//option[@value='" + value + "']"));
        option.click();
      } else if (field.contains("duration")) {
        WebElement input = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("form#booking-form input[type='number']")));
        input.clear();
  ...
  public void verifySuccessMessage(String expectedMessage) {
    WebElement successMessage = wait.until(ExpectedConditions.visibilityOfElementLocated(
        By.cssSelector(".feedback.success")));
